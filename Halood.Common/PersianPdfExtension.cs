...
public static class PersianPdfExtension
{
    private static Dictionary<string, string[]> _PersianGlyphs;

    static Regex _RegIsEnglish = new Regex("^[a-zA-Z0-9\\.,!@#$%\\^&\\*\\?()<>'\"]*$");

    static PersianPdfExtension()
    {
        _PersianGlyphs= InitializeTableUnicode();
    }
    private static Dictionary<string, string[]> InitializeTableUnicode()
    {
        ...
        xGlyphs.Add("\\u0622", new string[] { "\\u0622", "\\u0622", "\\uFE82", "\\uFE82", "2" });// (آ) Alef maddah
        ...
        xGlyphs.Add("\\u0628", new string[] { "\\u0628", "\\uFE91", "\\uFE92", "\\uFE90", "4" });// (ب) Beh
        ...
        xGlyphs.Add("\\u0634", new string[] { "\\u0634", "\\uFEB7", "\\uFEB8", "\\uFEB6", "4" });// (ش) Sheen
        ...
        xGlyphs.Add("\\u06CC", new string[] { "\\u06CC", "\\uFEF3", "\\uFEF4", "\\uFEF0", "4" });// (ی) Yeh
        xGlyphs.Add("\\u066E", new string[] { "\\uFBE4", "\\uFBE8", "\\uFBE9", "\\uFBE5", "4" });// (ٮ)  Dotless Beh 
        xGlyphs.Add("\\u06A9", new string[] { "\\uFB8E", "\\uFB90", "\\uFB91", "\\uFB8F", "4" });// (ڪ) Swash Kaf 
        xGlyphs.Add("\\u06AF", new string[] { "\\u06AF", "\\uFB94", "\\uFB95", "\\uFB93", "4" });// (گ) Swash Kaf 
        xGlyphs.Add("\\u067E", new string[] { "\\u067E", "\\uFB58", "\\uFB59", "\\uFB57", "4" });// (پ) Peh
        xGlyphs.Add("\\u0686", new string[] { "\\u0686", "\\uFB7C", "\\uFB7D", "\\uFB7B", "4" });// (چ) Che
        xGlyphs.Add("\\u0698", new string[] { "\\u0698", "\\u0698", "\\uFB8B", "\\uFB8B", "2" });// (ژ) Zhe
        xGlyphs.Add("\\u06C1", new string[] { "\\uFBA6", "\\uFBA8", "\\uFBA9", "\\uFBA7", "4" });// (ه) Heh Goal
        xGlyphs.Add("\\u06E4", new string[] { "\\u06E4", "\\u06E4", "\\u06E4", "\\uFEEE", "2" });// () Small High Madda 
        return xGlyphs;
    }

    public static Dictionary<string, string[]> GetPersianGlyphs()
    {
        return _PersianGlyphs;
    }
    ...
    private static string GetUnShapedUnicode(this string original)
    {
        ...
        var xUnicodeTable = GetPersianGlyphs();
    ...
    public static string PersianWithFontGlyphsToPfd(this string source)
    {
